...
@implementation UIView (RLGFrame)
...
@interface RLGPopupMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation RLGPopupMenuCell
...
#pragma mark - ///////////
#pragma mark - RLGPopupMenu

@interface RLGPopupMenu ()
<
UITableViewDelegate,
...
- (instancetype)initWithTitles:(NSArray *)titles
                     ...
                     menuWidth:(CGFloat)itemWidth
                      delegate:(id<RLGPopupMenuDelegate>)delegate
{
    if (self = [super init]) {
...
- (void)dismiss
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(RLGPopupMenuDidDismiss)]) {
        [self.delegate RLGPopupMenuDidDismiss];
    }
}
...
- (void)showRelyOnView:(UIView *)view scale:(CGFloat)scale{
    CGRect absoluteRect = [view convertRect:view.bounds toView:kMainWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    _mainView.layer.mask = [self getMaskLayerWithPoint:relyPoint];
    if (self.y < _anchorPoint.y) {
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<RLGPopupMenuDelegate>)delegate
{
    RLGPopupMenu *popupMenu = [[RLGPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<RLGPopupMenuDelegate>)delegate
{
    RLGPopupMenu *popupMenu = [[RLGPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
...
{
    static NSString * identifier = @"RLGPopupMenu";
    RLGPopupMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[RLGPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(RLGPopupMenuDidSelectedAtIndex:RLGPopupMenu:)]) {
        
        [self.delegate RLGPopupMenuDidSelectedAtIndex:indexPath.row RLGPopupMenu:self];
    }
}
...
{
    RLGPopupMenuCell *cell = [self getLastVisibleCell];
...
- (RLGPopupMenuCell *)getLastVisibleCell
{
...
- (void)setType:(RLGPopupMenuType)type
{
    _type = type;
    switch (type) {
        case RLGPopupMenuTypeDark:
        {
