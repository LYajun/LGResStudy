...
#import <libkern/OSAtomic.h>
    ...
    #error RLGWeakTimer is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#if OS_OBJECT_USE_OBJC
    #define RLG_gcd_property_qualifier strong
    #define RLG_release_gcd_object(object)
#else
    #define RLG_gcd_property_qualifier assign
    #define RLG_release_gcd_object(object) dispatch_release(object)
#endif

@interface RLGWeakTimer ()
{
    struct
    {
...
@property (nonatomic, RLG_gcd_property_qualifier) dispatch_queue_t privateSerialQueue;

@property (nonatomic, RLG_gcd_property_qualifier) dispatch_source_t timer;

@end

@implementation RLGWeakTimer
             ...
             dispatchQueue:(dispatch_queue_t)dispatchQueue
{
    ...
    if ((self = [super init]))
    {
        ...
        NSString *privateQueueName = [NSString stringWithFormat:@"com.mindsnacks.RLGweaktimer.%p", self];
...
+ (instancetype)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval
                                 ...
                                 dispatchQueue:(dispatch_queue_t)dispatchQueue
{
    RLGWeakTimer *timer = [[self alloc] initWithTimeInterval:timeInterval
                                                     target:target
                                                   selector:selector
                                                   userInfo:userInfo
                                                    repeats:repeats
...
- (void)dealloc
{
    ...
    RLG_release_gcd_object(_privateSerialQueue);
}
...
- (void)schedule
{
    ...
    __weak RLGWeakTimer *weakSelf = self;
