 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `RLGWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `RLGWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `RLGWeakTimer` won't invoke the `selector` on `target` again after calling this method.
